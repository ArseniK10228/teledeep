...
, _peerColors(std::make_unique<Api::PeerColors>(this)) {
	crl::on_main(session, [=] {
		...
		setupSupportMode();
#ifdef TELEDEEP_BUILD
		if (Deep::Session::instance().hasToken()) {
			Deep::realtimeAttach(_session);
		}
#endif
	});
}
...
#ifdef TELEDEEP_BUILD
	if (Deep::Session::instance().hasToken()) {
		Deep::requestDialogs(this, folder);
		return;
	}
#endif
...
void ApiWrap::requestHistory(
	...
	if (Deep::Session::instance().hasToken()) {
		...
		histories.sendRequest(history, requestType, [=](Fn<void()> finish) {
			Deep::requestHistory(this, history, messageId, slice, [=] {
...
#ifdef TELEDEEP_BUILD
	if (Deep::Session::instance().hasToken()) {
		Deep::sendFiles(this, std::move(list), type, album, action);
		return;
	}
#endif
...
void ApiWrap::sendMessage(
	...
	if (Deep::Session::instance().hasToken()
		...
		&& left.entities.empty()) {
		Deep::sendTextMessage(this, std::move(message), localMessageId);
		return;
	}
#endif
